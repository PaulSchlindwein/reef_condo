...
import { onResortCategories, Category } from '@/data/activities';

export default function OnResortActivitiesPage() {
  const [selectedCategory, setSelectedCategory] = useState<string | null>(null);
  const [sortBy, setSortBy] = useState<string>('name');
  const [filterType, setFilterType] = useState<string>('all');

  const currentCategory = selectedCategory 
    ? onResortCategories.find(cat => cat.id === selectedCategory)
    : null;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Hero Section */}
      <section className="relative h-64 bg-gradient-to-r from-ocean-blue to-blue-600">
        ...
        <div className="relative z-10 h-full flex items-center justify-center">
          <div className="text-center text-white">
            ...
            <p className="text-xl max-w-2xl mx-auto px-4">
              Discover everything Atlantis Resort has to offer - from world-class dining to thrilling water parks.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-8 px-4 bg-white border-b">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-wrap gap-4 justify-center">
            ...
            {onResortCategories.map((category) => (
              <button
                key={category.id}
                ...
                <div key={category.id}>
                  ...
                  {category.activities.length > 3 && (
                    <div className="text-center">
                      <button
                        onClick={() => setSelectedCategory(category.id)}
                        className="bg-ocean-blue hover:bg-blue-800 text-white px-6 py-2 rounded-lg font-semibold transition-colors duration-200"
                      >
            ...
            <div>
              ...
              <div className="bg-white p-4 rounded-lg shadow-md mb-8">
                <div className="flex flex-wrap gap-4 items-center">
                  ...
                  <button
                    onClick={() => setSelectedCategory(null)}
                    className="text-ocean-blue hover:text-blue-800 font-medium"
                  >
